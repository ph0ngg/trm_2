...
from .losses import IOUloss, triplet_loss, cross_entropy_loss
from .network_blocks import BaseConv, DWConv, CBAM
import numpy as np
...
class YOLOXHead(nn.Module):
    def __init__(
        ...
        self.linear = nn.Linear(self.embedding_dims, self.num_ids)
        ...
        for i in range(len(in_channels)):
            ...
            self.emb_convs.append(
                CBAM(channels= int(256*width), r= 4)
                # nn.Sequential(
                #     *[
                #         Conv(
                #             in_channels=int(256 * width),
            ...
            #     nn.Linear(
            #         in_features= self.embedding_dims,
            #         out_features = self.num_ids
            #     )
            # )
        ...
        # self.neck2 = nn.Conv2d(1024, 128, 1, 1)
        # self.neck1 = nn.Conv2d(512, 128, 1, 1)
        # self.neck0 = nn.Conv2d(256, 128, 1, 1)
        # self.cbam = CBAM(128, r=4)
    ...
    def forward(self, xin, labels=None, imgs=None):
        ...
        #print(targets.shape)
        ###
        # neck_feat_0 = self.cbam(self.neck0(xin[0]))
        # neck_feat_1 = self.cbam(self.neck1(xin[1]))
        # neck_feat_2 = self.cbam(self.neck2(xin[2]))
        ...
        ###        
        for k, (cls_conv, reg_conv, emb_conv, stride_this_level, x) in enumerate(
            zip(self.cls_convs, self.reg_convs, self.emb_convs, self.strides, xin)
        ):  
            x = self.stems[k](x)
            cls_x = x
            reg_x = x
            ...
            emb_feat = emb_conv(id_x)
            emb_output = self.emb_preds[k](emb_feat) #B, 128, H, W



            if self.training:
                output = torch.cat([reg_output, obj_output, cls_output, emb_output], 1)
                ...
                output = torch.cat(
                    [reg_output, obj_output.sigmoid(), cls_output.sigmoid(), emb_output], 1
                )
    ...
    def get_losses(
        ...
        for batch_idx in range(outputs.shape[0]):
            ...
            if num_gt == 0:
                ...
                gt_ids = labels[batch_idx, :num_gt, 1]
                ...
                id_target = gt_ids[matched_gt_inds]
                if self.use_l1:
...
#               
            ...
            #     total_triplet_loss += triplet_loss(emb_vectors_pred[:, :, i], gt_ids)
#           
        ...
        id_preds = self.linear(emb_preds.view(-1, 128)[fg_masks])

        loss_id = 0.5*triplet_loss(emb_preds.view(-1, 128)[fg_masks], id_targets) + 0.5*cross_entropy_loss(id_preds, id_targets.long())
        ...
        #loss_id = triplet_loss(emb_preds, id_targets)
    ...
    def get_assignments(
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            try:
                ...
                print("cls_preds_:", cls_preds_.unsqueeze(0).repeat(num_gt, 1, 1))
                print("gt_cls_per_image:", gt_cls_per_image.unsqueeze(1).repeat(1, num_in_boxes_anchor, 1))
